<template>
    <CollapsableFloatingCard class="absolute top-4 left-4 z-10" :icon="PhNavigationArrow" title="Rota">
        ...
        <div class="overflow-y-auto" style="max-height: 19rem">
            ...
            <div v-else-if="currentWaypoints.length === 0" class="flex flex-col items-center justify-center py-8 gap-1">
            ...
            <template v-else>
                <div v-for="(wp, index) in currentWaypoints" :key="wp.ident + index"
                    ...
                    <div class="flex flex-col items-center gap-0.5 w-5 shrink-0">
                        ...
                        <div v-if="index < currentWaypoints.length - 1" class="w-px flex-1 min-h-[6px] bg-white/10" />
                    </div>
