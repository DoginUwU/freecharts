<template>
  <div class="flex flex-col items-center text-center gap-2 w-full" ref="container">
    <CheckboxGroup :items="availableChartTypes" :selecteds="[state.currentChartType]" @select="handleChangeChartType" />

    <div v-for="(charts, subtype, index) in groupedCharts[state.currentChartType]" :key="index" class="w-full"
      :class="{ 'mt-2': index > 0 }">
      <span v-if="subtype !== 'null'">{{ subtype }}</span>
      <ul class="w-full mt-4 relative">
        <TransitionGroup name="charts-list" appear>
          <li v-for="(chart, index) in charts" :key="chart.id"
            class="chart-type rounded-lg w-full flex items-center justify-start bg-zinc-900/70 outline-none p-2 transition-colors group"
            :class="{
              active: selectedChart?.id === chart.id,
              'mt-2': index > 0,
            }" role="button" @click="emit('loadChart', chart)">
            <PhAirplaneTilt class="inline-block mr-2" />
            <span class="text-white text-start">
              {{ chart.name.toUpperCase() }}
            </span>
            <PhStar role="button" class="inline-block ml-auto opacity-0 group-hover:opacity-100 transition-opacity"
              :class="{
                '!opacity-100':
                  chartsStore.isFavoritedChart(chart),
              }" :color="chartsStore.isFavoritedChart(chart) ? '#facc15' : '#fff'"
...
import { PhAirplaneTilt, PhStar } from "@phosphor-icons/vue";
